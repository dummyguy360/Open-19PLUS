function scr_enemy_walk()
{
    if (grounded)
        hsp = (image_xscale * movespeed) + (5 * (place_meeting(x, y + 1, obj_railh2) - place_meeting(x, y + 1, obj_railh)));
    else if (object_index != obj_ancho)
        hsp = 0;
    
    if (floor(image_index) == 2 && grounded)
        instance_create_depth(x - (image_xscale * 20), y + 43, 0, obj_cloudeffect);
    
    sprite_index = walkspr;
    image_speed = 0.35;
    
    if (scr_solid(x + image_xscale, y) || place_meeting(x + hsp, y, obj_hallway))
    {
        if (!place_meeting(x + sign(hsp), y, obj_slope))
        {
            if (object_index == obj_forknight)
            {
                image_xscale *= -1;
                image_index = 0;
                sprite_index = spr_forknight_turn;
                state = states.tackleold;
            }
            else
            {
                image_xscale *= -1;
            }
        }
    }
    
    if (object_index != obj_ancho)
    {
        if (!(scr_solid(x + (image_xscale * 15), y + 31) || place_meeting(x + (image_xscale * 15), y + 31, obj_platform)))
        {
            if (movespeed > 0 && grounded)
            {
                if (object_index == obj_forknight)
                {
                    image_xscale *= -1;
                    image_index = 0;
                    sprite_index = spr_forknight_turn;
                    state = states.tackleold;
                }
                else
                {
                    image_xscale *= -1;
                }
            }
        }
    }
}
