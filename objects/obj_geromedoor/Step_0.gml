...
if (place_meeting(x, y, p) && input_check("up") && p.grounded && (p.state == 0 || p.state == 47 || p.state == 66 || p.state == 42) && sprite_index == spr_geromedoorunopened && there_is_follower_obj(obj_gerome))
{
...
with (p)
{
    ...
    if (place_meeting(x, y, obj_geromedoor) && input_check("up") && grounded && other.sprite_index == spr_geromedooropened && (state == 0 || state == 47 || state == 66) && !instance_exists(obj_fadeout) && state != 54 && state != 41 && state != 39)
    {
        image_index = 0;
    ...
    if (floor(image_index) == (image_number - 1) && state == 41)
    {
        targetDoor = other.targetDoor;
        targetRoom = other.targetRoom;
