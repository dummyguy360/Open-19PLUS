...
if (baddieID.state != 107 && baddieID.state != 300)
{
    with (__player)
    {
        ...
        if (character == "S" && _bid.stuntouchbuffer <= 0 && (state == 47 || state == 66) && sprite_index != spr_jump)
        {
...
if (__player.state != 91 && __player.state != 50)
{
    if (baddieID.state != 107 && baddieID.state != 300)
    {
        with (__player)
        {
            ...
            if (instakillmove && state != 87 && !_bid.ishurt)
            {
                if ((state == 66 && (character != "S" || character == "S")) || (state == 48 && character != "S" && (sprite_index == spr_machslideboost3 || sprite_index == spr_machslideboost3end)) || state == 18 || state == 13 || (state == 105 && (sprite_index == spr_barrelroll || sprite_index == spr_barrelslipnslide)) || state == 1 || state == 96 || state == 8 || state == 4)
                {
                    with (_bid)
                    {
                        if (hp <= 0)
                    ...
                    if (state == 66 && sprite_index != spr_mach3hit && sprite_index != spr_crazyrun && !rocket && sprite_index != spr_player_sjumpcancel)
                    {
                ...
                else
                {
                    ...
                    if (state == 66 && sprite_index != spr_mach3hit && !rocket && sprite_index != spr_player_sjumpcancel)
                    {
                        sprite_index = spr_mach3hit;
                        image_index = 0;
                    }
                    ...
                    if (input_check("jump"))
                    {
                        if (!grounded && state != 51 && state != 25 && !rocket && sprite_index != spr_freefall && sprite_index != spr_facestomp)
                            vsp = -11;
                    }
                    else if (!grounded && vsp > 0 && state != 120 && state != 25 && state != 51 && state != 5 && sprite_index != spr_freefall && sprite_index != spr_facestomp)
                    {
                        vsp = 0;
                    }
                    ...
                    if (state == 51 || sprite_index == spr_freefall || sprite_index == spr_facestomp)
                        other.alarm[0] = 60;
                    else
                        other.alarm[0] = global.mach2hitstun + 8;
                    
                    if (character == "S" || _bid.hp <= 0 || hitstunstoredstate == 51 || hitstunstoredstate == 75 || hitstunstoredstate == 25 || hitstunstoredstate == 5 || hitstunstoredstate == 88 || hitstunstoredstate == 120 || hitstunstoredstate == 129 || hitstunstoredsprite != spr_freefall || hitstunstoredsprite != spr_facestomp)
                    {
            ...
            if ((_bid.state == 104 || _bid.state == 92) && state == 0 && global.currentbadge == 1 && !_bid.thrown && !_bid.ishurt)
            {
                _bid.state = 107;
                _bid.grabbedby = id;
                state = 119;
            ...
            if (!grounded && y < _bid.y && !attacking && vsp > 0 && _bid.vsp >= 0 && sprite_index != spr_mach2jump && sprite_index != spr_swingding && sprite_index != spr_swingdingend && sprite_index != spr_stompprep && (state == 36 || state == 81 || state == 106 || state == 28))
            {
                if (x != _bid.x)
            ...
            if (_bid.state != 107 && (state == 25 || state == 51))
            {
                _bid.sprite_index = _bid.stunfallspr;
                _bid.flash = 1;
